...
AliAnalysisTaskPolarizationTestJP::AliAnalysisTaskPolarizationTestJP() : AliAnalysisTaskSE(),
  //events
  fMuonTrackCuts(0x0),fIsMC(0),fAOD(0),fRunNumber(0),fTrgRunNum(0),fMC(0),fTracklets(0)
  //output list
  ,fOutputList(0)
  //trees
...
void AliAnalysisTaskPolarizationTestJP::UserExec(Option_t *)
{
  ...
  iSelectionCounter++;
  fCounterH->GetXaxis()->SetBinLabel(iSelectionCounter,"nocuts");
...
fCounterH->Fill(iSelectionCounter);
iSelectionCounter++;
fCounterH->GetXaxis()->SetBinLabel(iSelectionCounter,"ADSelection");
...
//Int_t jSelectionCounter = iSelectionCounter;
AliAODTrack *track;
Int_t paircounter = 0;
...
//cout<<"entering the track loop"<< iTrack<<endl;
  // get track
track = static_cast<AliAODTrack*>(fAOD->GetTrack(iTrack));
if(!track) return;
...
// this is the trigger definition from simone's code
  /*   Int_t                   fV0ADecision;   //!
     Int_t                   fV0CDecision;
    AliVVZERO *dataVZERO = dynamic_cast<AliVVZERO*>(fAOD->GetVZEROData());
    fV0ADecision = dataVZERO->GetV0ADecision();
...
*/
// end of trigger definition from simone's code



/* - This is implemented using Evgeny's Code to create VZero and AD triggers
   - I decided to use this as it is shorter and gave the same result.
 */
